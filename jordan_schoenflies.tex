...
\begin{remark}[Polygonal sets and their vertex presentations]\label{rem:polygonal-presentation}
\emph{Polygonal} is the set-level condition just defined --- a finite
union of line segments --- and every statement below quantifies over
polygonal sets in that sense, with no condition on any list of
vertices. This is deliberate: the constructions produce sets, not
vertex lists. A formal development, however, will represent a
polygonal curve by a vertex list, and will want that list to have no
three consecutive collinear vertices, since that is what the germ
argument at a vertex in the proof of Lemma~\ref{lem:polygonal-collar}
runs on. The two notions are not interchangeable for free. Deleting
redundant collinear vertices, the opening move of that proof, is an
operation on presentations, and the class of lists without collinear
triples is not closed under the splices performed later: at a join
vertex in Theorem~\ref{thm:polygonal-crosscut} the crosscut may leave
along the ray directly opposite the arriving edge --- perfectly
compatible with its meeting the curve only at its two endpoints --- so
that the spliced curve is a simple closed polygonal curve whose
inherited presentation has a collinear triple. What is owed, and is
not stated below because at the set level there is nothing to state,
is a realization theorem: every simple polygonal arc and every simple
closed polygonal curve admits a vertex presentation without collinear
triples.
\end{remark}
...
\begin{lemma}[Recognizing a component]\label{lem:clopen-component}
Let $V\subseteq\mathbb R^2$ be open and let $U\subseteq V$ be a region
with $\partial U\cap V=\varnothing$. Then $U$ is a connected component
of $V$.
\end{lemma}
\begin{proof}
The set $U$ is open, hence open in $V$. It is also closed in $V$,
because
$\overline U\cap V=(U\cup\partial U)\cap V=U\cap V=U$. Let $K$ be the
component of $V$ containing the nonempty connected set $U$. Then $U$ is
a nonempty subset of $K$ which is both open and closed in $K$, and $K$
is connected, so $U=K$.
\end{proof}
...
For a directed edge with unit tangent $u$, write
$u^{\perp}=(-u_2,u_1)$ for $u$ turned counterclockwise through a right
angle, so that $\det(u,u^{\perp})=1$, and call the $u^{\perp}$ side of
the edge its left side. We claim that at a vertex the left sides of the
incoming and outgoing strips enter the same component of the vertex disk
minus the two incident rays.

Let $u$ be the incoming unit tangent and $w$ the outgoing one, so the
two rays leave the vertex in the directions $r_1=-u$ and $r_2=w$.
Neither $r_1=r_2$ nor $r_1=-r_2$ occurs. The first says $w=-u$, so the
two incident segments overlap in a nondegenerate initial segment,
contrary to simplicity; the second says $w=u$, a collinear vertex
deleted at the start. So the two rays are distinct and not opposite, and
they bound two arcs of directions at the vertex.

Points of the left half-strip of the incoming edge near the vertex lie
in directions $-tu+su^{\perp}$ with $t,s>0$ and $s/t$ small, and for
such a direction $d$, bilinearity and $\det(u,u)=0$ give
\[
 \det(r_1,d)=\det(-u,-tu+su^{\perp})=-s<0 .
\]
Points of the left half-strip of the outgoing edge lie in directions
$tw+sw^{\perp}$ with $t,s>0$ and $s/t$ small, and there
\[
 \det(r_2,d)=\det(w,tw+sw^{\perp})=s>0 .
\]
So the incoming left germ lies immediately clockwise of $r_1$ and the
outgoing left germ immediately counterclockwise of $r_2$; by the
direction facts of \hyperref[app:background]{Appendix~C} both therefore
lie on the arc traversed counterclockwise from $r_2$ to $r_1$, the first
just before its end and the second just after its start. The two right
germs reverse both signs and lie on the other arc. No angle is named and
no case distinction on the sense of the turn arises.
...
determinant calculation shows left joins to left at every vertex. Thus the
...
so that no edge is horizontal; this is possible because the finitely many
edge directions exclude only finitely many rotations
(\hyperref[app:background]{Appendix~C}). For
...
The count is a sum over edges satisfying an inequality, not a count of
the points in which a ray meets $C$, and it is well defined for every
$q\notin C$ without any genericity assumption. Because no edge is
horizontal, each counted edge meets the line $y=\eta$ in exactly one
point, and no edge lies along that line. A vertex at height $\eta$ is
handled by the half-open condition: both of its edges meet $y=\eta$ at
the vertex itself, so they stand or fall together on the test ``strictly
to the right of $q$'', and of the two, one is counted if the curve runs
through the vertex \textup(the arriving edge has $b_y=\eta$ and the
departing edge $a_y=\eta$\textup), two at a local minimum, and none at a
local maximum. The comparison ``strictly to the right'' is never a tie,
since an edge meeting $y=\eta$ at $q$ itself would place $q$ on $C$.

We record one further property for use in
Theorem~\ref{thm:polygonal-crosscut}.

\begin{lemma}[Subdivision invariance]\label{lem:parity-subdivision}
Let $C'$ be obtained from $C$ by replacing an edge with upward endpoints
$a,b$ by the two edges from $a$ to $c$ and from $c$ to $b$, where $c$ is
an interior point of that edge. Then $\pi_{C'}=\pi_C$ on
$\mathbb R^2\setminus C$.
\end{lemma}
\begin{proof}
Neither new edge is horizontal, and $a_y<c_y<b_y$, so the conditions
$a_y\le\eta<c_y$ and $c_y\le\eta<b_y$ are mutually exclusive and their
disjunction is $a_y\le\eta<b_y$. Hence for each $\eta$ at most one of
the two new edges is counted, and one is counted exactly when the old
edge was. Each new edge is a subset of the old one, so when it is
counted it meets $y=\eta$ in the same point that the old edge did, and
the test against $q$ has the same outcome. All other edges are
unchanged.
\end{proof}
...
one unbounded, and both have boundary $C$. Moreover $\pi_C=1$ on the
bounded region $\operatorname{Int}(C)$ and $\pi_C=0$ on the unbounded
region $\operatorname{Ext}(C)$.
\end{theorem}
\begin{proof}
...
For the parity values, take a point $q$ whose first coordinate exceeds
that of every point of the compact set $C$. No edge crosses the
horizontal line through $q$ strictly to the right of $q$, so
$\pi_C(q)=0$; and $q$ lies in the unbounded region, since the
complement of a large disk is connected and disjoint from $C$. As
parity is constant on each region and the two regions have opposite
parity, $\pi_C=0$ on the unbounded region and $\pi_C=1$ on the bounded
one.
...
\begin{definition}[Separating Jordan curve]\label{def:separating}
A Jordan curve $C$ is \emph{separating} if $\mathbb R^2\setminus C$ has
exactly two regions, one bounded and one unbounded, and each has
boundary $C$. The bounded region is written $\operatorname{Int}(C)$ and
the unbounded one $\operatorname{Ext}(C)$.
\end{definition}

Theorem~\ref{thm:polygonal-jordan} says that every polygonal Jordan
curve is separating, and Theorem~\ref{thm:jordan} will remove the word
``polygonal''. The next two lemmas are needed on both sides of that
divide. They are therefore proved from Definition~\ref{def:separating}
alone, and are never invoked for a curve whose separation has not
already been established.

\begin{lemma}[Absorption]\label{lem:absorption}
Let $C$ and $J$ be separating Jordan curves, let $\Omega$ be a region
of $\mathbb R^2\setminus C$, and let $V$ be a region of
$\mathbb R^2\setminus J$ with $\Omega\subseteq V$. Then
$C\setminus J\subseteq V$.
\end{lemma}
\begin{proof}
Since $C$ is separating, $C=\partial\Omega$, so
$C\subseteq\overline\Omega$. From $\Omega\subseteq V$ we get
$\overline\Omega\subseteq\overline V$, and since $J$ is separating,
$\overline V=V\cup\partial V=V\cup J$. Hence $C\subseteq V\cup J$.
\end{proof}

\begin{lemma}[Crosscut cells]\label{lem:crosscut-cells}
Let $C$ be a separating Jordan curve and let $P$ be a simple arc whose
endpoints $p\ne q$ lie on $C$ and which has no other point on $C$. Let
$A_1,A_2$ be the two arcs of $C$ from $p$ to $q$ and put $J_i=A_i\cup
P$; these are Jordan curves, and we assume them separating. Let
$\Omega$ be the region of $\mathbb R^2\setminus C$ containing
$P\setminus\{p,q\}$ and $\Omega^\dagger$ the other region. For each
$i$, let $W_i$ be the region of $\mathbb R^2\setminus J_i$ containing
$\Omega^\dagger$, and let $V_i$ be the other region. Then
\begin{enumerate}\alphlabels
\item $V_1$ and $V_2$ are connected components of $\Omega\setminus P$;
\item $V_1\ne V_2$;
\item $\overline{V_i}\cap C=A_i$.
\end{enumerate}
\end{lemma}
\begin{proof}
Each $A_i$ is a simple arc from $p$ to $q$ meeting $P$ exactly in
$\{p,q\}$, so each $J_i$ is a Jordan curve. The set $P\setminus\{p,q\}$
is connected and disjoint from $C$, so it lies in one region of
$\mathbb R^2\setminus C$; this defines $\Omega$. The region
$\Omega^\dagger$ is disjoint from $C$ and from $P$, hence from
$J_i\subseteq C\cup P$; being connected it lies in one region of
$\mathbb R^2\setminus J_i$, so $W_i$ and $V_i$ are well defined.

Apply Lemma~\ref{lem:absorption} to $\Omega^\dagger\subseteq W_i$: it
gives $C\setminus J_i\subseteq W_i$, a set disjoint from $V_i$. The
remaining part of $C$, namely $C\cap J_i=A_i$, lies in $J_i$ and is
also disjoint from $V_i$. Hence $V_i\cap C=\varnothing$.

So $V_i$ is a region contained in $\mathbb R^2\setminus C$ and disjoint
from $\Omega^\dagger$; being connected, it lies in $\Omega$. It is also
disjoint from $P\subseteq J_i$. Therefore
$V_i\subseteq\Omega\setminus P$. Its boundary is $J_i\subseteq C\cup
P$, which is disjoint from the open set $\Omega\setminus P$, so
Lemma~\ref{lem:clopen-component} makes $V_i$ a component of
$\Omega\setminus P$. This proves (a).

For (b), suppose $V_1=V_2$. Taking boundaries gives $J_1=J_2$, and
removing $P\setminus\{p,q\}$ from both sides gives
$A_1\cup\{p,q\}=A_2\cup\{p,q\}$, that is, $A_1=A_2$. But
$A_1\cap A_2=\{p,q\}$, while each $A_i$ is a nondegenerate arc; so
$A_1=A_2$ would force $C=A_1=\{p,q\}$, which is absurd.

For (c), $\overline{V_i}=V_i\cup J_i$ and $V_i\cap C=\varnothing$, so
$\overline{V_i}\cap C=J_i\cap C=A_i$.
\end{proof}
...
\begin{lemma}[Parity splitting]\label{lem:parity-splitting}
Let $C$ be a simple closed polygonal curve, let $P$ be a simple
polygonal arc with distinct endpoints $p,q\in C$ and no other point on
$C$, let $A_1,A_2$ be the two arcs of $C$ from $p$ to $q$, and put
$J_i=A_i\cup P$. Subdivide $C$ at $p$ and $q$ and rotate coordinates so
that no edge of $C\cup P$ is horizontal. Then at every point off
$C\cup P$,
\begin{equation}\label{eq:parity-splitting}
 \pi_C=\pi_{J_1}+\pi_{J_2}\pmod 2.
\end{equation}
Consequently a point of $\operatorname{Int}(C)$ off $P$ lies inside
exactly one of $J_1,J_2$, and a point of $\operatorname{Ext}(C)$ off
$P$ lies inside both or inside neither.
\end{lemma}
\begin{proof}
Subdividing changes no parity function, by
Lemma~\ref{lem:parity-subdivision}. After the subdivision the edge set
of $C$ is the disjoint union of the edge sets of $A_1$ and $A_2$, and
the edge set of $J_i$ is that of $A_i$ together with that of $P$.
Counting edge by edge, each edge of $P$ contributes once to
$\pi_{J_1}$ and once to $\pi_{J_2}$, so its contributions cancel modulo
$2$; the remaining contributions to $\pi_{J_1}+\pi_{J_2}$ are those of
the edges of $A_1$ and $A_2$, which are exactly the contributions to
$\pi_C$. This is \eqref{eq:parity-splitting}.

By Theorem~\ref{thm:polygonal-jordan} the parity function of each of
the three curves is $1$ on its bounded region and $0$ on its unbounded
region. So at a point of $\operatorname{Int}(C)$ off $P$ the left side
of \eqref{eq:parity-splitting} is $1$ and exactly one $\pi_{J_i}$ is
$1$; at a point of $\operatorname{Ext}(C)$ off $P$ the left side is $0$
and the two agree.
\end{proof}
...
\end{longtheorem}
\begin{proof}
All three curves are separating, by
Theorem~\ref{thm:polygonal-jordan}. Let $\Omega$ be the region of
$\mathbb R^2\setminus C$ containing $P\setminus\{p,q\}$, let
$\Omega^\dagger$ be the other region, and let $W_i,V_i$ be as in
Lemma~\ref{lem:crosscut-cells}. That lemma exhibits $V_1$ and $V_2$ as
two distinct components of $\Omega\setminus P$; only exhaustion
remains, and it is read off Lemma~\ref{lem:parity-splitting}.

Suppose first that $P\setminus\{p,q\}\subseteq\operatorname{Int}(C)$,
so $\Omega=\operatorname{Int}(C)$ and
$\Omega^\dagger=\operatorname{Ext}(C)$. The latter is connected,
unbounded and disjoint from $J_i$, so it lies in
$\operatorname{Ext}(J_i)$; hence $W_i=\operatorname{Ext}(J_i)$ and
$V_i=\operatorname{Int}(J_i)$. Every point of
$\operatorname{Int}(C)\setminus P$ lies inside exactly one $J_i$, that
is, in exactly one $V_i$. Therefore
$\operatorname{Int}(C)\setminus P=V_1\sqcup V_2$, which is (a).

Suppose instead that $P\setminus\{p,q\}\subseteq\operatorname{Ext}(C)$,
so $\Omega=\operatorname{Ext}(C)$ and
$\Omega^\dagger=\operatorname{Int}(C)$. The latter lies inside exactly
one of $J_1,J_2$; relabel so that it lies inside $J_1$. Then
$W_1=\operatorname{Int}(J_1)$ and $V_1=\operatorname{Ext}(J_1)$, while
$W_2=\operatorname{Ext}(J_2)$ and $V_2=\operatorname{Int}(J_2)$. A point
of $\operatorname{Ext}(C)\setminus P$ lies inside both $J_i$ or inside
neither; in the first case it lies in $V_2$, in the second in $V_1$.
Therefore $\operatorname{Ext}(C)\setminus P=V_1\sqcup V_2$, with
boundaries $\partial V_1=J_1$ and $\partial V_2=J_2$, which is (b).

In either case $\Omega^\dagger$ is a region disjoint from $C\cup P$
whose boundary $C$ misses $\mathbb R^2\setminus(C\cup P)$, so
Lemma~\ref{lem:clopen-component} makes it a component of that set.
Since
\[
 \mathbb R^2\setminus(C\cup P)
 =\bigl(\Omega\setminus P\bigr)\ \sqcup\ \Omega^\dagger
 =V_1\sqcup V_2\sqcup\Omega^\dagger,
\]
the three regions are $V_1,V_2,\Omega^\dagger$, with boundaries
$J_1,J_2,C$.
\end{proof}
...
\item If $P$ is a path whose two endpoints are distinct vertices of
      $G$, then $G\cup P$ is 2-connected. The internal vertices of $P$
      need not be new.
\end{enumerate}
\end{longlemma}
\begin{proof}
...
For (b), delete a vertex $x$. The graph $G-x$ is connected --- it is
$G$ itself if $x$ is not a vertex of $G$ --- and it still contains an
endpoint of $P$, since the two endpoints are distinct vertices of $G$
and at most one of them is $x$. Every vertex of $P-x$ is joined along
$P$ to a surviving endpoint of $P$: if $x$ is internal to $P$, the two
remaining subpaths contain one endpoint each; if $x$ is an endpoint of
$P$, what remains of $P$ is a path containing the other endpoint; and
if $x$ does not lie on $P$, nothing of $P$ is lost. Every vertex of
$(G\cup P)-x$ therefore reaches a surviving endpoint of $P$, and those
endpoints lie in the connected graph $G-x$. Thus removing any vertex
leaves $G\cup P$ connected.
\end{proof}

\begin{remark}[Where freshness is needed]\label{rem:ear-freshness}
The freshness of the internal vertices belongs to (a), not to (b).
Half (a) is the case of ``replace an edge of $G$ by a path between its
two endpoints'' in which the replacement path is new, and there
freshness is essential: without it the deleted vertex may lie both on
the replacement path and inside $G$, and then neither route between
the replaced edge's ends need survive. Half (b) deletes nothing, and
what carries its argument is only that every vertex of $P-x$ still
reaches an endpoint of $P$. Ears produced by an overlay, as several
below are, carry no freshness certificate --- their internal vertices
are whatever the overlay created --- and (b) applies to them
unchanged.
\end{remark}
...
\end{lemma}
\begin{proof}
Let $H_0$ be the graph constructed so far. If a component $L$ of
$G-V(H_0)$ contains a vertex, it has at least two distinct neighbors in
$H_0$; otherwise its unique neighbor would be a cut vertex of $G$. A
...
on one side of $C$ and is a crosscut there. The point $x$ lies in a
face of $G$, while $R\cup C\subseteq|G|$, so $x$ lies on neither of
the two cycles $R\cup C_1$, $R\cup C_2$; by
Theorem~\ref{thm:polygonal-crosscut}, exactly one of them encloses
$x$. That cycle remains in the fixed
...
\begin{remark}[The descent step needs $x$ off the crosscut]\label{rem:outer-chain-descent}
The step that replaces $C$ by one of $R\cup C_1$, $R\cup C_2$ is the
one a formal development will want to isolate as a lemma about a
cycle, a crosscut of it, and an enclosed point --- and in that form it
is false unless the point is required to lie off the crosscut. Take
the unit square for $C$, cut at $(0,0)$ and $(1,1)$, let $R$ be the
two segments from $(0,0)$ to $(1/2,1/2)$ to $(1,1)$, and let
$x=(2/5,2/5)$. Every condition on the cycle and the crosscut holds and
$x$ is enclosed by $C$, but $x$ lies on $R$, hence on both spliced
curves, and is enclosed by neither. In the proof above the clause is
free, since a point in a face of $G$ is off $|G|$ and both spliced
curves lie in $|G|$; stated on its own, the step must carry it.
\end{remark}
...
\begin{lemma}[Model-curve parametrization]\label{lem:jordan-circle}
Let $C$ be a Jordan curve with parametrization $\gamma$, and let
$S=\partial Q$ be the boundary of the square $Q=[-1,1]^2$. Then $\gamma$
induces a homeomorphism from $S$ onto $C$. Consequently,
...
\end{lemma}
\begin{proof}
Let $e:[0,1]\to S$ traverse the four sides of $S$ at constant speed:
$e$ is affine on each interval $[k/4,(k+1)/4]$ and carries the five
endpoints to the four corners of $Q$ in cyclic order, returning to the
first. It is a continuous surjection onto $S$ which identifies $0$ with
$1$ and is injective otherwise. Since $\gamma(0)=\gamma(1)$, setting
$h(e(t))=\gamma(t)$ gives a well-defined bijection $h$ from $S$ onto
$C$. It is continuous: if $z_n\to z$ in $S$, choose $t_n$ with
...
about $S$, where they are immediate from the piecewise-affine
description of $e$. Nothing in this document needs the unit circle in
place of $S$; taking the model curve to be $\partial Q$ keeps the whole
development free of trigonometry, and $S$ is in any case the curve that
Part~II maps onto.
\end{proof}
...
of $C\cap L_i$. Let $C_1,C_2$ be the two arcs of $C$ from
$p_1$ to $p_2$. Choose a vertical line $L_3$ strictly between
$L_1,L_2$. Both arcs meet $L_3$, and the compact disjoint sets
$C_1\cap L_3$ and $C_2\cap L_3$ have positive distance along the
line. Choose closest points $a\in C_1\cap L_3$ and
$b\in C_2\cap L_3$. The open segment between them misses $C$; denote
...
$\{a,b,d'\}$. The four subarcs of $C_1,C_2$, the two subsegments of
...
\end{theorem}
\begin{proof}
By Theorem~\ref{thm:jordan} the curve $C$ is separating, with
$D=\operatorname{Int}(C)$; each $J_i=A_i\cup P$ is a Jordan curve and
so is separating too. Apply Lemma~\ref{lem:crosscut-cells} with
$\Omega=D$. Here $\Omega^\dagger=\operatorname{Ext}(C)$ is connected,
unbounded and disjoint from $J_i$, hence lies in
$\operatorname{Ext}(J_i)$; so $W_i=\operatorname{Ext}(J_i)$ and
$V_i=\operatorname{Int}(J_i)$. The lemma therefore exhibits
$\operatorname{Int}(J_1)$ and $\operatorname{Int}(J_2)$ as two distinct
components of $D\setminus P$. By
Lemma~\ref{lem:crosscut-at-most-two} there are at most two components,
so these are both of them.
...
\]
The region $\operatorname{Ext}(C)$ has boundary $C$, which misses
$\mathbb R^2\setminus(C\cup P)$, so it is a component by
Lemma~\ref{lem:clopen-component}; the other two are components of
$D\setminus P$ and their boundaries $J_1,J_2$ miss
$\mathbb R^2\setminus(C\cup P)$ likewise. This proves the three-region
statement together with the asserted boundaries.
\end{proof}
...
homeomorphisms
\[
\Phi:C\cup\operatorname{Int}(C)\longrightarrow Q,
\qquad
\Psi:C'\cup\operatorname{Int}(C')\longrightarrow Q.
\]
...
There is a countable dense set $\mathcal A\subseteq C$ every point of which is
...
$u(a)$ with $a\in\mathcal A$, and that exactly one new nonboundary edge is
...
If \(|L|\setminus C\) has more than one component, choose points in two of its components and join them by a simple polygonal arc in \(D\), using Lemma~\ref{lem:polygonal-connected}. Overlay this polygonal arc with the polygonal nonboundary skeleton. Each component of the arc outside the old graph is an open subarc whose closure has two distinct endpoints on the old graph; after subdivision it is an ear. By Lemma~\ref{lem:subdivision-ear-preserve}, adding these ears successively preserves 2-connectivity. Because the joining arc lies in \(D\), every ear endpoint lies in the open nonboundary part; hence no ear increases the number of components of the open nonboundary part, and once all ears of one joining arc are present, the whole arc lies in the open nonboundary part and joins the two chosen components. Each round therefore strictly decreases the number of components. There are only finitely many of them, and this must be argued rather than assumed: \(L\) is a finite plane graph, \(C\) is a union of edges of \(L\), and distinct edges of a plane graph meet only at shared vertices, so every edge of \(L\) not contained in \(C\) meets \(C\) in at most its two endpoints. Hence \(|L|\setminus C\) is the union of the finitely many open nonboundary edges and the finitely many nonboundary vertices, and it has at most that many components. Finitely many repetitions therefore produce a 2-connected graph \(H_n\) for which \(|H_n|\setminus C\) is connected.

The graph \(H_n\) contains the entire local grid and has all the stated properties. \end{proof}

\begin{remark}[Finiteness of the component count is not a plane fact]\label{rem:grid-attachment-finiteness}
The finiteness just used is a property of the overlay, not of the
plane. A single straight segment can meet a wild Jordan curve in a
Cantor set, and the complement of that set in the segment has
uncountably many components; so ``\(|L|\setminus C\) has finitely many
components'' is unavailable from the geometry of \(L\) and \(C\)
alone. What supplies it is the plane-graph invariant recorded above:
every polygonal object fed into the overlay meets \(C\) in at most two
points, because \(C\) is carried as a union of edges and distinct
edges meet only at shared vertices. The convention of
Remark~\ref{rem:polygonal-overlay-convention} is what keeps that
invariant available, since it never overlays anything against \(C\)
itself.
\end{remark}
...
Let \(V_\partial\subseteq S\) be the finite set of boundary vertices already present. At stage \(n\), only finitely many points of \(\mathcal A\) have previously been used. The set \(\mathcal A\) is dense in \(C\) by Proposition~\ref{prop:countable-strong-access}, so \(u(\mathcal A)\) is dense in \(S\); removing the images of previously used points and the finite set \(V_\partial\) leaves a dense subset of \(S\): if a nonempty relatively open arc of \(S\) met the dense set in only finitely many points, deleting those points would leave a nonempty open subarc disjoint from the dense set, contradicting density; hence every nonempty open arc meets the dense set in infinitely many points, and finitely many deletions cannot remove them all.
...
\item
  every new internal edge of \(T\) meeting \(S\) ends at a fresh point of \(u(\mathcal A)\);
\item
...
The inner grid is 2-connected by the same ear construction used for \(K\), together with Lemma~\ref{lem:subdivision-ear-preserve}. Because \(m\ge2\), the two endpoints \(\lambda z_i\), \(\lambda z_{i+1}\) of the inner boundary arc of a collar rectangle are distinct, so the boundary of every collar rectangle is a cycle sharing with the inner grid at least those two vertices. Adding these finitely many cycles one at a time and applying Lemma~\ref{lem:union-two-connected} proves that the full mesh is 2-connected. After deleting \(S\), the inner grid together with all open spokes is still connected. The only inward edge incident with \(S\) at a new point is the single spoke at that point. \end{proof}

\begin{remark}[Two spokes are needed, and two are enough]\label{rem:mesh-two-spokes}
Clause~5 is false if fewer than two fresh points are chosen, and for
large \(\delta\) the arc-diameter condition permits that choice, so
\(m\ge2\) has to be imposed outright. With no fresh point there is no
spoke, and the mesh is the outer cycle together with the inner grid,
two disjoint pieces --- not even connected. With exactly one, the
single spoke is the only thing joining those two pieces, so every
interior vertex of the spoke is a cut vertex. Two are already enough,
which is what the argument above uses: from \(m\ge2\) onwards every
collar rectangle shares two distinct vertices with the inner grid. In
the application \(\delta=\varepsilon_n=2^{-n}\), and the arc-diameter
condition then forces far more than two.
\end{remark}
...
Let \(\mathcal A_\infty\subseteq\mathcal A\) be the set of preimages under $u$ of all
fresh target anchors used over all stages. Then \(u(\mathcal A_\infty)\) is
dense in \(S\) and \(\mathcal A_\infty\) is dense in \(C\). Moreover, at every
stage containing a given point of \(\mathcal A_\infty\), that point is incident
...
Let \(\mathcal A_\infty\subseteq\mathcal A\) be the set of anchor preimages of
Lemma~\ref{lem:anchor-density}; it is dense in \(C\), every
\(a\in\mathcal A_\infty\) is incident with a nonboundary edge at every stage
...
Let \(a,b\in\mathcal A_\infty\) be distinct anchors occurring in a common finite stage. Then that stage contains a polygonal crosscut \(P\) of \(D\) from \(a\) to \(b\), and its image under the finite skeleton homeomorphism is a polygonal crosscut \(P'\) of \(Q^\circ\) from \(u(a)\) to \(u(b)\).
\end{lemma}
\begin{proof}
...
Let \(P\) be a crosscut contained in a finite source skeleton, with endpoints \(a,b\in\mathcal A_\infty\), and let \(P'\) be its target image. Let \(A_1,A_2\) be the two boundary arcs of \(C\) from \(a\) to \(b\), and label the components \(U_1,U_2\) of \(D\setminus P\) by
...
Put \(r=u^{-1}(q)\), so \(r\ne p\). Since \(\mathcal A_\infty\) is dense, choose \(a,b\in\mathcal A_\infty\), distinct from \(p,r\), such that \(p\) and \(r\) lie in opposite components of \(C\setminus\{a,b\}\). The stages are nested, so \(a\) and \(b\) occur in a common finite stage; let \(P\) and \(P'\) be the crosscuts supplied by Lemma~\ref{lem:skeleton-crosscuts}.
...
\[
\Theta:C'\cup\operatorname{Int}(C')\longrightarrow Q.
\]

By Lemma~\ref{lem:square-point-mover}, choose a boundary-fixing homeomorphism of \(Q\) sending \(\Theta(\Phi(a))\) to \(\Theta(b)\). Conjugating it by \(\Theta\) and composing with \(\Phi\) changes the image of \(a\) to \(b\) without changing the map on \(C\). \end{proof}
...
such that \(\Phi^*(a)=b\). It therefore restricts to a homeomorphism after the two points \(a,b\) are removed. Conjugating that restriction by \(I_a\) and \(I_b\), using Lemma~\ref{lem:inversion-sides}, gives the required exterior extension. On \(C\) the conjugated map is exactly \(f\). \end{proof}
...
Remark~\ref{rem:polygonal-presentation}\newline\emph{Polygonal sets and their vertex presentations} & Lemma~\ref{lem:polygonal-collar} and Theorem~\ref{thm:polygonal-crosscut} \\
...
Lemma~\ref{lem:clopen-component}\newline\emph{Recognizing a component} & Definitions and imported background (\hyperref[app:background]{Appendix~C}) only. \\
Lemma~\ref{lem:polygonal-collar}\newline\emph{Two-sided polygonal strips} & Lemma~\ref{lem:compact-separation} \\
Lemma~\ref{lem:parity-subdivision}\newline\emph{Subdivision invariance} & Definitions and imported background (\hyperref[app:background]{Appendix~C}) only. \\
...
Definition~\ref{def:separating}\newline\emph{Separating Jordan curve} & Definitions and imported background (\hyperref[app:background]{Appendix~C}) only. \\
Lemma~\ref{lem:absorption}\newline\emph{Absorption} & Definitions and imported background (\hyperref[app:background]{Appendix~C}) only. \\
Lemma~\ref{lem:crosscut-cells}\newline\emph{Crosscut cells} & Lemma~\ref{lem:absorption} and Lemma~\ref{lem:clopen-component} \\
Lemma~\ref{lem:parity-splitting}\newline\emph{Parity splitting} & Lemma~\ref{lem:parity-subdivision} and Theorem~\ref{thm:polygonal-jordan} \\
Theorem~\ref{thm:polygonal-crosscut}\newline\emph{Two-sided polygonal crosscuts} & Theorem~\ref{thm:polygonal-jordan}, Lemma~\ref{lem:crosscut-cells}, Lemma~\ref{lem:parity-splitting} and Lemma~\ref{lem:clopen-component} \\
...
Remark~\ref{rem:ear-freshness}\newline\emph{Where freshness is needed} & Definitions and imported background (\hyperref[app:background]{Appendix~C}) only. \\
...
Remark~\ref{rem:outer-chain-descent}\newline\emph{The descent step needs $x$ off the crosscut} & Definitions and imported background (\hyperref[app:background]{Appendix~C}) only. \\
...
Lemma~\ref{lem:jordan-circle}\newline\emph{Model-curve parametrization} & Definitions and imported background (\hyperref[app:background]{Appendix~C}) only. \\
...
Theorem~\ref{thm:general-crosscut}\newline\emph{Crosscut theorem} & Lemma~\ref{lem:jordan-circle}, Theorem~\ref{thm:jordan}, Lemma~\ref{lem:crosscut-cells}, Lemma~\ref{lem:crosscut-at-most-two} and Lemma~\ref{lem:clopen-component} \\
...
Remark~\ref{rem:grid-attachment-finiteness}\newline\emph{Finiteness of the component count is not a plane fact} & Remark~\ref{rem:polygonal-overlay-convention} \\
Proposition~\ref{prop:anchored-square-mesh}\newline\emph{Anchored square mesh} & Lemma~\ref{lem:jordan-circle}, Remark~\ref{rem:mesh-two-spokes}, Lemma~\ref{lem:subdivision-ear-preserve} and Lemma~\ref{lem:union-two-connected} \\
Remark~\ref{rem:mesh-two-spokes}\newline\emph{Two spokes are needed, and two are enough} & Definitions and imported background (\hyperref[app:background]{Appendix~C}) only. \\
...
\item \textbf{Plane and metric.} The plane with the Euclidean metric;
      ...
      points of a line fixes that line pointwise; the orientation form
      \(\det(a,b)=a_1b_2-a_2b_1\) and the right-angle rotation
      \(u\mapsto u^{\perp}=(-u_2,u_1)\), for which
      \(\det(u,u^{\perp})=\|u\|^2\); the fact that two distinct,
      non-opposite unit vectors bound two open arcs of directions, and
      that a direction near the first with negative \(\det\) against it
      lies on the same arc as a direction near the second with positive
      \(\det\) against it; the existence of a unit vector avoiding any
      prescribed finite set of directions; a nondegenerate compact connected
...
\section{Concordance with the compact companion}\label{app:concordance}

The compact companion states its results under the names in the left
column. A reader of that text who wants a step expanded should consult
the statements listed opposite. The two documents do not correspond one
to one: the compact text bundles the whole limit construction into a
single proposition, while this one splits several arguments that the
compact text shares between two places. Every named statement of the
compact text appears here.

Notation agrees across the two documents. In particular $\mathcal A$ is
the strongly accessible set, $A_1,A_2$ are arcs of a Jordan curve,
$C_1,C_2$ are arcs of a graph cycle, $P$ is a crosscut, $G,H,\Gamma$
are graphs, $K$ is a compact set, and homeomorphisms between domains
are capital Greek.

\begin{longtable}{@{}p{0.40\textwidth}p{0.52\textwidth}@{}}
\textbf{Compact companion} & \textbf{This document}\\
\hline
\endfirsthead
\textbf{Compact companion} & \textbf{This document}\\
\hline
\endhead
\emph{Relative Jordan--Schönflies} & Theorem~\ref{thm:main} \\
\emph{Compact separation} & Lemma~\ref{lem:compact-separation}, Lemma~\ref{lem:nearest-segment} \\
\emph{Nested compact singleton} & Lemma~\ref{lem:nested-compact}, Lemma~\ref{lem:diameter-closure} \\
\emph{Recognizing a component} & Lemma~\ref{lem:clopen-component} \\
\emph{Polygonal connectedness} & Lemma~\ref{lem:polygonal-connected}, Lemma~\ref{lem:finite-polygonal-union} \\
\emph{Two-sided strips} & Lemma~\ref{lem:polygonal-collar} \\
\emph{The count has no degenerate cases} (remark) & Lemma~\ref{lem:parity-subdivision} \\
\emph{Crossing parity} & Lemma~\ref{lem:polygon-parity} \\
\emph{Polygonal Jordan curve theorem} & Theorem~\ref{thm:polygonal-jordan} \\
\emph{Absorption} & Lemma~\ref{lem:absorption} \\
\emph{Crosscut cells} & Definition~\ref{def:separating}, Lemma~\ref{lem:crosscut-cells} \\
\emph{Parity splits} & Lemma~\ref{lem:parity-splitting} \\
\emph{Polygonal crosscuts} & Theorem~\ref{thm:polygonal-crosscut} \\
\emph{Alternating crosscuts meet} & Corollary~\ref{cor:alternating-crosscuts} \\
Untitled graph-facts lemma, parts (a)--(f) & Lemma~\ref{lem:cycle-criterion}, Lemma~\ref{lem:three-leaf-tree}, Lemma~\ref{lem:union-two-connected}, Lemma~\ref{lem:subdivision-ear-preserve}, Lemma~\ref{lem:relative-ear}, Lemma~\ref{lem:polygonal-overlay} \\
\emph{Polygonal redrawing} & Lemma~\ref{lem:polygonal-redrawing} \\
Untitled $K_{3,3}$ lemma & Lemma~\ref{lem:k33}, Corollary~\ref{cor:k33-subdivision} \\
\emph{Face cycles} & Lemma~\ref{lem:face-cycles} \\
\emph{Outer face of a chain} & Lemma~\ref{lem:outer-chain} \\
\emph{Arc-complement theorem} & Theorem~\ref{thm:arc-complement} \\
\emph{Circular parametrization} & Lemma~\ref{lem:jordan-circle} \\
\emph{A Jordan curve separates} & Proposition~\ref{prop:jordan-disconnected} \\
\emph{Accessible points are dense} & Lemma~\ref{lem:accessible-dense} \\
\emph{Jordan curve theorem} & Theorem~\ref{thm:jordan} \\
\emph{At most two sides} & Lemma~\ref{lem:crosscut-at-most-two} \\
\emph{Crosscut theorem} & Theorem~\ref{thm:general-crosscut} \\
\emph{Accessible endpoints can be joined} & Lemma~\ref{lem:accessible-endpoints} \\
\emph{Square extension} & Theorem~\ref{thm:square-extension} \\
\emph{Reduction to the square} & Proposition~\ref{prop:square-reduction} \\
``Strong accessibility'' paragraph & Definition~\ref{def:strong-accessibility}, Lemma~\ref{lem:nearest-strong}, Lemma~\ref{lem:tangent-dense}, Lemma~\ref{lem:tangent-cone}, Proposition~\ref{prop:countable-strong-access} \\
\emph{Initial pair} & Proposition~\ref{prop:initial-pair} \\
Definition of a cellulation and its two constructors & Definition~\ref{def:admissible-graph}, Definition~\ref{def:matched-pair}, Definition~\ref{def:matched-cellulation}, Definition~\ref{def:generated-structure}, Remark~\ref{rem:intermediate-disconnection} \\
\emph{Cell geometry} & Lemma~\ref{lem:cellulation-invariants} \\
\emph{Order, parents, stars} & Lemma~\ref{lem:cellulation-invariants}, Lemma~\ref{lem:outer-incidence} \\
\emph{Correspondence} & Lemma~\ref{lem:combinatorial-invariance}, Lemma~\ref{lem:outer-incidence}, Lemma~\ref{lem:star-intersection} \\
\emph{Stars} & Lemma~\ref{lem:star-face-mesh}, Lemma~\ref{lem:refinement-compatibility} \\
\emph{Local sectors and face access} & Lemma~\ref{lem:local-skeleton-structure}, Lemma~\ref{lem:polygonal-side-accessibility} \\
\emph{Finite transfer} & Theorem~\ref{thm:finite-transfer} \\
\emph{Local grid} & Proposition~\ref{prop:local-grid-attachment} \\
\emph{Anchored mesh} & Proposition~\ref{prop:anchored-square-mesh} \\
\emph{Shrinking stars} & Lemma~\ref{lem:grid-star-estimate}, Proposition~\ref{prop:shrinking-stars} \\
Density of the anchor set & Lemma~\ref{lem:anchor-density} \\
\emph{The interior map} & Lemma~\ref{lem:cell-neighborhood}, Proposition~\ref{prop:skeleton-agreement}, Proposition~\ref{prop:F-continuous}, Proposition~\ref{prop:image-interior}, Proposition~\ref{prop:F-injective}, Proposition~\ref{prop:target-skeleton-dense}, Proposition~\ref{prop:F-surjective}, Lemma~\ref{lem:exact-cell-correspondence}, Proposition~\ref{prop:inverse-continuous}, Proposition~\ref{prop:interior-homeomorphism} \\
\emph{Skeleton crosscuts} & Lemma~\ref{lem:skeleton-crosscuts} \\
\emph{Sides correspond} & Lemma~\ref{lem:crosscut-side-correspondence} \\
\emph{Boundary continuity} & Proposition~\ref{prop:boundary-continuity}, Theorem~\ref{thm:closed-interior-extension} \\
\emph{Moving a point of the square} & Lemma~\ref{lem:square-point-mover} \\
\emph{Pointed extension} & Proposition~\ref{prop:pointed-extension} \\
\emph{Inversion} & Lemma~\ref{lem:inversion-sides}, Proposition~\ref{prop:exterior-extension} \\
\end{longtable}
