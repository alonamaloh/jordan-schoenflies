...
can have positive area, and it can meet a line in a Cantor set. No
construction on it may rely on local regularity, and every proof of the
theorem is therefore a proof that the monstrosity is irrelevant: that
the curve is controlled entirely by objects one \emph{can} compute
with. Here those
...
The route is Thomassen's~\cite{Thomassen}; the work here is to leave no
step unargued.
...
distinct: $V_1=V_2$ would give $J_1=\partial V_1=\partial V_2=J_2$ and
hence $A_1=J_1\setminus P^\circ=J_2\setminus P^\circ=A_2$, which is
impossible because $A_1\cap A_2=\{p,q\}$ while each $A_i$ is a
nondegenerate arc. Finally
...
relatively open neighborhoods $O_e\supseteq K_e$ meeting the square
boundaries only in small disjoint arcs around the marks: the boundary
minus such an arc is compact and disjoint from $K_e$. Compact
separation does not deliver connectedness, so let $U_e$ be the
component of $O_e$ containing the connected set $K_e$; components of
relatively open subsets of the locally connected $M$ are relatively
open. The
...
The quantitative work in Sections~\ref{sec:scheme}
and~\ref{sec:interior} is carried entirely by stars, so we record what
they do.

\begin{lemma}[Stars]\label{lem:stars}
\textup{(a)} Every cell is a subcell of a face, so that
$\St(\sigma)$ is the union of the closures of the faces above $\sigma$;
and if each of those closed faces has diameter $<\eta$ then
$\diam\St(\sigma)<2\eta$.
\textup{(b)} Parents preserve the subcell order.
\textup{(c)} Hence $\St(\sigma^{+})\subseteq\St(\sigma)$ whenever
$\sigma$ is the parent of $\sigma^{+}$; and the parent of the carrier
of a point is its carrier at the previous stage, so the closed stars of
a fixed point shrink, weakly, under refinement.
\end{lemma}
\begin{proof}
(a) Both constructors preserve the property: initially every cell lies
in $\overline{R_1}$ or $\overline{R_2}$; a subdivision's new cells lie
in $\overline e$, and $e$ is below a face by hypothesis; a split's new
cells are the crosscut and the two new faces, and the crosscut is below
both. If now $\tau\succeq\sigma$ and $\tau\preceq R$ with $R$ a face,
then $\overline\tau\subseteq\overline R$ and $R\succeq\sigma$, which
gives the stated description of $\St(\sigma)$. Every closed face in it
contains $\sigma$, hence a fixed point $x\in\sigma$; two points of the
star are therefore within $2\eta$ of each other through $x$.

(b) In each constructor all new cells share one parent $\rho$ --- the
subdivided edge, or the split face --- and every surviving cell is its
own parent. A relation between two new cells gives $\rho\preceq\rho$;
between two old cells, an old relation. If an old $\sigma$ satisfies
$\sigma\preceq\tau$ with $\tau$ new, then
$\sigma\subseteq\overline\tau\subseteq\overline\rho$, so
$\sigma\preceq\rho$ as required. The remaining case cannot occur: a new
cell below a surviving cell would put it in an old closed cell other
than $\overline\rho$, which the proof of
Lemma~\ref{lem:cellorder}(b) excludes.

(c) For $\tau^{+}\succeq\sigma^{+}$ with parent $\tau$, we have
$\overline{\tau^{+}}\subseteq\overline\tau$ and, by (b),
$\tau\succeq\sigma$; so $\overline{\tau^{+}}\subseteq\St(\sigma)$.
For the carriers: if $x$ has carriers $\sigma$ before and $\sigma^{+}$
after, then $\sigma^{+}$ meets $\overline\sigma$, so
$\sigma^{+}\subseteq\overline\sigma$ by the frontier property and
$\operatorname{par}(\sigma^{+})\preceq\sigma$; and
$x\in\overline{\operatorname{par}(\sigma^{+})}$ forces
$\sigma\preceq\operatorname{par}(\sigma^{+})$ the same way. Antisymmetry
finishes it.
\end{proof}
...
\emph{inner} edge meeting $S$ has exactly one endpoint on $S$, at a
fresh point of $u(\mathcal A)$ that becomes a boundary vertex with
exactly one incident new inner edge. \textup(The new \emph{outer} edges
are unrestricted: subdividing $S$ at a fresh point creates two of them,
both with both endpoints on $S$.\textup)
\end{theorem}
\begin{proof}
...
(Theorem~\ref{thm:transfer}(a)); refine \emph{that} target skeleton ---
the one carrying the transferred grid, not $\Gamma_{n-1}'$ --- by the
...
Let $G'$ be the skeleton of any cellulation of $Q$, and let
$\delta>0$. Then $G'$ has a refinement $T$, a polygonal cellulation of
$Q$ whose faces have diameter $<\delta$, whose skeleton is 2-connected
with $|T|\setminus S$ connected, and whose only new inner edges meeting
$S$ are single \emph{spokes}, one at each of finitely many fresh points
of $u(\mathcal A)$. \textup(It is applied below to the target skeleton
current at that moment, not to $\Gamma_{n-1}'$.\textup)
\end{proposition}
\begin{proof}
Radial coordinates $x=tz$ ($t=\|x\|_\infty$, $z\in S$) identify the
collar $Q\setminus(\lambda Q)^\circ$ with $S\times[\lambda,1]$; fix
$0<\lambda<1$ with $\sqrt2(1-\lambda)<\delta/4$. Parametrize $S$ by a
...
$u(\mathcal A)$ in the relative interior of each, avoiding the finitely
many boundary vertices of $G'$ --- possible since a dense set meets every open subarc
even after finitely many deletions. Consecutive $z_i$ then bound arcs
$A_i\subseteq S$ of diameter $<\delta/4$; insert the boundary vertices
of $G'$ as subdivisions of these arcs. Draw the spokes
...
attached. Overlaying $T$ with $G'$ preserves 2-connectivity, since both graphs
contain the outer cycle $S$ and so share at least two vertices
(Lemma~\ref{lem:graphfacts}(c)); if the two inner parts are disjoint,
adding the ears of one joining arc in $Q^\circ$, as in
Proposition~\ref{prop:grid}, connects them. This yields the required
refinement; the only new inner edges meeting $S$ are the spokes, and
the new outer edges are the subedges into which $S$ has been cut.
\end{proof}
...
refinement
(each face lies in the closure of its parent), so by
Lemma~\ref{lem:stars}(a) the star diameters are $<2\varepsilon_n$.
...
$\overline T$ in its closure, of diameter $<\varepsilon_n$. All closed faces of $\St_{\Gamma_n}(x)$ contain $x$, so
$\diam\St_{\Gamma_n}(x)<2\varepsilon_n$ by Lemma~\ref{lem:stars}(a);
and by Lemma~\ref{lem:stars}(c) stars only shrink at later stages.
\end{proof}
...
\bibitem{Thomassen}
C. Thomassen, \emph{The Jordan--Schönflies theorem and the
classification of surfaces}, Amer. Math. Monthly \textbf{99} (1992),
116--130.
\end{thebibliography}
